...
MODEL = 'AB_len60_301dim_w2v_in_and_out.h5'
REINFORCE = False
...
class neural_chatbot():
    ...
    def model_response(self,User_B):
        ...
        User_A = w2v.vectorize( self.out, pad_length = MAX_SENT_LENGTH,model=self.w2v_model )[:MAX_SENT_LENGTH]
        
        #Cat the two sentences together one after another
        Input_AB = np.concatenate((User_A,self.User_B_in),axis=1)
        self.user_input = [ Input_AB.reshape(1,MAX_SENT_LENGTH*2,EMBED_DIM) ]
